...
public class HttpClientUtil {
    ...
    public static String postFile(String url, String keyName, List<Map<String, Object>> files) {
        if (url == null || StringUtils.isEmpty(url)) {
            return null;
        }
        // 创建HttpClient实例
        ...
        try {
            ...
            for (Map<String, Object> item : files) {
                ...
                builder.addBinaryBody(keyName, (InputStream) item.get("inputStream"), ContentType.MULTIPART_FORM_DATA, (String) item.get("fileName"));// 文件流

            }

            HttpEntity entity = builder.build();
            hp.setEntity(entity);
            responseStr = httpclient.execute(hp, responseHandler);// 执行提交
        } catch (IOException e) {
            e.printStackTrace();
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public static void main(String[] args) {
        ...
        map.put("orderId", "10170802321506671629");

        String result1 = post("http://101.132.117.76/mobile/api/client/v1/queryAvailableCouponList", map);

        System.out.println("result1 ------------ " + result1);

        Map<String, String> map2 = new HashMap<>();

        map2.put("cdKey", "FAF12949E98E6458");
        map2.put("userId", "1017080232");

        String result = post("http://101.132.117.76/mobile/api/client/v1/exchangeCoupon", map2);

        System.out.println("result -------------" + result);

        String result2 = post("http://101.132.117.76/mobile/api/client/v1/queryAvailableCouponList", map);

        System.out.println("result2 -------------" + result2);

       /* List<Map<String, Object>> files = new ArrayList<>();

        File readFile = new File("/Users/frank/Downloads/1234.png");
        ...
        try {
            ...
            //is = new ByteArrayInputStream(IOUtils.toByteArray(input));

            Map<String, Object> file = new HashMap<>();
            file.put("fileName", readFile.getName());
            file.put("inputStream", is);
            files.add(file);

            String res = postFile("http://106.14.169.84/mobile/api/system/v1/uploadFile", "file", files);
